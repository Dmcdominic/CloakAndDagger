...
public abstract class config_fields_controller<T0, T1, T2> : MonoBehaviour	where T0 : struct, System.IConvertible
	...
	protected void Start () {
		// TODO - Remove this - Fields should be generated by the UI button(s) that open the settings
		create_all_fields();
	}
	...
	public void create_all_fields() {
		...
		create_fields(config.ui_parameters_ordered, new List<bool_input>(), interactable);
	}
		...
		foreach (object option in ui_parameters.Keys) {
			if (option is T0) {
				...
				if (config.ui_dependents != null && config.ui_dependents.ContainsKey(T0_option)) {
					...
					create_fields(config.ui_dependents[T0_option], dependencies, interactable);
