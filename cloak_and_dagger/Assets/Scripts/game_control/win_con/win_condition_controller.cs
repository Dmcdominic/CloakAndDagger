...
public abstract class win_condition_controller : sync_behaviour<win_con_init_struct> {
...
#pragma warning disable 0414
	...
	private void on_done_initing() {
		if (WCAP.host.val) {
			...
			send_state(new win_con_init_struct(true, false));
		}
	}

	public override void rectify(float t, win_con_init_struct state) {
		if (state.loaded_and_ready_msg && WCAP.host.val) {
			loaded_and_ready_msgs_received++;
		...
		if (state.start_countdown_msg && !WCAP.host.val) {
	...
	IEnumerator on_all_ready_host_only() {
		if (WCAP.host.val) {
			send_state(new win_con_init_struct(false, true));
	...
	// It will send the start_countdown_msg after a backup delay.
